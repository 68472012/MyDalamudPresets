...
configs:
  ...
  Dodging Plugin:
    ...
    description: |
      使用するAOE回避プラグインを入力。
      BossModRebornまたはBossModを使用する場合、この設定は上書きされます。
      入力可能な選択肢:
        - Any
        - BossMod
        - BossModReborn
  ...
  Single Target Rotation:
    ...
    type: string
    description: |
     対単体用スキル回しプリセット名を入力(フォーローン/フォーローンメイデン向け)。
  ...
  Percentage to Hold Buff:
    ...
    description: |
     ...
     バーストはリキャスト毎に使用するのが理想ですが、FATE進行度が70%を超えてからバーストした場合、討伐が早すぎて数秒無駄になることがあります。
     デフォルト: 65
     入力できる値: 整数
...
function SelectNextFateHelper(tempFate, nextFate)
    ...
    elseif BonusFatesOnly then
        ...
        -- WaitForBonusIfBonusBuffはコメント内の変数の変更忘れでshouldWaitForBonusBuffのこと
